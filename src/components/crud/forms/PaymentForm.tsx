...
import { paymentField } from "../formtype/general";
...
const PaymentForm: React.FC<DealerFormProps> = (props: any) => {
  const data = props.data;
  const formType = props.formType;
  const [submitting, setSubmitting] = useState(false);
  const formField =
    data._id ? populateFormFields(paymentField, data) : paymentField;
  const [formData, setFormData] = useState<any>(
    data._id ? populateFormData(paymentField, data) : {}
  );
  ...
  const { user, amount, upiId, status, createdAt, _id } = data;

  return (
    <div>
      <div className="bg-white rounded-2xl mb-5 p-6 w-full border border-gray-200">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">Transaction #{_id.slice(-6)}</h2>
          <span
