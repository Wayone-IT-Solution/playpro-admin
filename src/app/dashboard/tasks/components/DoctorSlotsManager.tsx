...
import { FiPlus, FiTrash2, FiCalendar, FiClock, FiUser, FiSave, FiEye, FiCopy } from 'react-icons/fi';
...
const DoctorSlotsManager = () => {
    ...
    const fetchGroundSlots = useCallback(async () => {
        try {
            ...
            const response: any = await Fetch(
                "/api/slot",
                { groundId: selectedDoctor, ...(selectedDate ? { date: selectedDate } : {}) },
                5000,
                true,
                false
            );
    ...
    const handleDaySelection = (slotId: number, day: string, isSelected: boolean) => {
        setSlots(slots.map(slot => {
            if (slot.id === slotId) {
                ...
                return { ...slot, selectedDays: updatedDays, selectedDates: updatedDates.sort() };
            }
            return slot;
        }));
    };
    ...
    const addDateToSlot = (slotId: number, dateInput: string) => {
        ...
        setSlots(slots.map(slot => {
            if (slot.id === slotId) {
                ...
                return { ...slot, selectedDates: updatedDates, selectedDays: updatedDays };
            }
            return slot;
        }));
    };
    ...
    const validateSlots = (): boolean => {
        return slots.every(slot => {
            const hasValidTime = slot.startTime && slot.endTime && slot.duration >= 15 && slot.duration <= 180;
    ...
    function getUpcomingDateByDay(dayName: string, fromDate: Date = new Date()): Date {
        const days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    ...
    const handleDateRangeAdd = (slotId: number, startDate: string, endDate: string) => {
            ...
            <div className="w-full">
                <div className="flex w-full items-center justify-between mb-6">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h1 className="text-xl font-extrabold text-gray-900">Ground Slots Manager</h1>
                            <p className="text-gray-600 text-xs">Create and manage doctor availability slots for multiple days</p>
                        </div>
                    </div>
                    ...
                    <>
                        {slots.length === 0 ? (
                            <div className="text-center py-12">
                                ...
                                <h3 className="text-lg font-medium text-gray-900 mb-2">No slots added yet</h3>
                                <p className="text-gray-600 mb-4">Click &apos;Add Slot&apos; to start creating doctor availability slots</p>
                            </div>
                        ) : (
                                ...
                                {slots.map((slot, index) => (
                                    <div key={slot.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                        <div className="flex items-center justify-between mb-4">
                                            <h3 className="text-lg font-semibold text-gray-900">Slot #{index + 1}</h3>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    value={slot.groundId}
                                                    onChange={(e) => updateSlot(slot.id, 'groundId', e.target.value)}
                                                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                                >
                                                    <option value="">Select Ground</option>
                                                    {grounds.map((ground: any, index) => (
                                                        <option key={index} value={ground._id}>
                                                            {ground.name} - {ground.firstName} {ground.lastName} ({ground.email})
                                                        </option>
                                                    ))}
                                                </select>
                                            </div>
                                            ...
                                            <div className="flex space-x-4">
                                                <label className="flex items-center">
                                                    <input
                                                        ...
                                                        onChange={() => updateSlot(slot.id, 'useRecurring', true)}
                                                        className="mr-2"
                                                    />
                                                    Recurring Days
                                                </label>
                                                <label className="flex items-center">
                                                    <input
                                                        ...
                                                        name={`scheduleType_${slot.id}`}
                                                        checked={!slot.useRecurring}
                                                        onChange={() => updateSlot(slot.id, 'useRecurring', false)}
                                                        className="mr-2"
                                                    />
                                                    Specific Dates
                                                </label>
                                        ...
                                        {slot.useRecurring && (
                                            <div className="mb-4">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {daysOfWeek.map(day => (
                                                        <label key={day.value} className="flex items-center space-x-2 p-2 border bg-white rounded-lg">
                                                            <input
                                                                type="checkbox"
                                                                checked={slot.selectedDays.includes(day.value)}
                                                                onChange={(e) => handleDaySelection(slot.id, day.value, e.target.checked)}
                                                                className="rounded"
                                                            />
                                            ...
                                            <div className="mb-4">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mb-3">
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => {
                                                            const startInput = document.getElementById(`startDate_${slot.id}`) as HTMLInputElement;
                                                            const endInput = document.getElementById(`endDate_${slot.id}`) as HTMLInputElement;
                                                            handleDateRangeAdd(slot.id, startInput.value, endInput.value);
                                                            startInput.value = '';
                                                            endInput.value = '';
                                                        }}
                                                ...
                                                <div className="flex gap-2 mb-3">
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => {
                                                            const dateInput = document.getElementById(`singleDate_${slot.id}`) as HTMLInputElement;
                                                ...
                                                {slot.selectedDates.length > 0 && (
                                                    <div className="border rounded-lg p-3 bg-white max-h-32 overflow-y-auto">
                                                        <div className="flex flex-wrap gap-1">
                                                            {slot.selectedDates.map((date) => (
                                                                <span
                                                                    ...
                                                                    onClick={() => handleDateSelection(slot.id, date)}
                                                                >
                                                                    {date}
                                                        ...
                                                        <div className="text-xs text-gray-500 mt-2">
                                                            {slot.selectedDates.length} date(s) selected • Click to remove
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        <div className="text-xs text-gray-600 bg-white p-2 rounded">
                                            <strong>Summary:</strong> This will create {
                                                ...
                                                slot.useRecurring
                                                    ? `recurring ${slot.selectedDays.map(day => daysOfWeek.find(d => d.value === day)?.label).join(', ')}`
                                                    : `specific dates`
                                            }
                                        </div>
                                    </div>
                        ...
                        {slots.length > 0 && (
                            <div className="mt-4 flex justify-between items-center">
                                <div className="text-sm text-gray-600">
                                    Total slots to create: <strong>{prepareApiData().length}</strong>
                                </div>
                                <button
                ...
                {showPreview &&
                    <div className='w-full '>
                        ...
                        <SlotBookingUI slots={previous} fetchGroundSlots={fetchGroundSlots} selectedDoctor={selectedDoctor} />
                    </div>
                }
            </div>
        </div>
