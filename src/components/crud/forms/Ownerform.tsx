...
import { ownerFormType } from "../formtype/userFormType";
...
const OwnerForm: React.FC<DealerFormProps> = (props: any) => {
  const data = props.data;
  const formType = props.formType;
  const [loading, setLoading] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [formField, setFormFields] = useState(data._id
    ? populateFormFields(ownerFormType, data, disabled, removed)
    : ownerFormType);

  const [formData, setFormData] = useState<any>(
    data._id ? populateFormData(ownerFormType, data) : { role: "admin" }
  );
